...
car_queues = {'east': [],'west': [],
              'south': [],'north': []} #contains the cars that are in a certain important segment of the road
...
def update_queues(cars, list_of_queues):
...
def generate_traffic(direction = None, selected_starting_point = None): #randomly generates a car
    ...
    queue = str(selected_starting_point)
    # print(starting_positions_cord)
    # print(starting_position)
    cars.append(Car(id, acceleration, length, direction, max_v, starting_position, dt, height, width, queue))
...
while True:
    ...
    if sig[0].check_flag == 1:  #  Whenever southern light does full rotation lights calibrate themselves.
        current = update_queues(cars, car_queues)
        # print(car_queues)
        # print(current)
