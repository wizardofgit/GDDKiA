...
Flow_table = {"Optimized": [], "Non-optimized": []}
...
for i in range(30):
    ...
    for j in range(2):
        ...
        time_threshold_step = 0.5 #time_threshold will be updated, so that cars are generated every time_threshold_step seconds
        minimum_cars_passed = 100 #number of cars which passed after which the simulation ends
        optimization_interval = 0.1  # Amount of seconds between optimizer scans
        ...
        while True:
            for event in pygame.event.get():    #check what user did
                if event.type == pygame.QUIT:   #if user clicked 'x' (aka quit) in window
                    ...
                    if j == 0:
                        Flow_table["Optimized"].append(cars_passed / time_elapsed * 3600)
                    else:
                        Flow_table["Non-optimized"].append(cars_passed / time_elapsed * 3600)
                    break  #stop the app
