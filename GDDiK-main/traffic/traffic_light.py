...
class TrafficLight:
    ...
    def __init__(self, light_id, position="horizontal", light_times_dict=None, status=None, countdown=0, check_flag =0,
                 internal_clock= 0):

        self.id = light_id  # Used to identify which signaling device we are referring to.

        self.position = position  # Used to differentiate between horizontal and vertical lights.
        ...
        self.check_flag = check_flag  # Used to start the scan and optimize the light timers
    ...
    def return_check_flag(self): # Returns the "check flag" for optimization.
...
def create_signalisation():  # Utility function to create default set of signaling devices.
...
def substitute(new_times_dict, signalisation):  # Substitutes light timers for existing lights
    horizontal_timers = {"red": new_times_dict["vertical"]*120, "green": new_times_dict["horizontal"]*120}
    vertical_timers = {"red": new_times_dict["horizontal"]*120, "green": new_times_dict["vertical"]*120}
    for signaling_device in signalisation:
